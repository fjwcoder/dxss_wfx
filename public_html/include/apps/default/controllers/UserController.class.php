...
class UserController extends CommonController {
    ...
    public function user_wechat_by_id($id){
        $sql = "SELECT * FROM ".$this->model->pre."wechat_user where ect_uid=".$id;
        $row = $this->model->query($sql);
        
        return $row[0];
    }

    // 账户中心 ：add by fjw in 18.4.23
    public function profile(){
        // 用户资料
        $user_info = model('Users')->get_profile($this->user_id);
        
        $profile = $this->user_wechat_by_id($this->user_id);
        $profile['user_name'] = $user_info['user_name'];
        $profile['mobile_phone'] = $user_info['mobile_phone'];
        $profile['birthday'] = $user_info['birthday'];

        if(!isset($profile['sex'])){ // 微信没有设置了性别
    ...
    public function edit_nickname(){
        $nickname = isset($_POST['nickname']) ? in($_POST['nickname']) : null;
        if($nickname === null){
            echo json_encode(array('status'=>false, 'content'=>'昵称为空')); exit();
        }
        $sql = "UPDATE ".$this->model->pre."wechat_user set nickname='".$nickname."' where ect_uid=".$this->user_id;
        $result = $this->model->query($sql);
        echo json_encode(array('status'=>true, 'nickname'=>$nickname));
    ...
    public function edit_birthday(){
        $birthday = isset($_POST['birthday']) ? in($_POST['birthday']) : null;
        if($birthday === null){
            echo json_encode(array('status'=>false, 'content'=>'生日数据错误')); exit();
        }

        $sql = "UPDATE ".$this->model->pre."users set birthday='".$birthday."' where user_id=".$this->user_id;
        $result = $this->model->query($sql);
        echo json_encode(array('status'=>true, 'birthday'=>$birthday));
    }
    ...
    //     // 修改个人资料的处理
    //     if (IS_POST) { // 这一块儿基本用不到了 注释 by fjw in 18.4.25
    //         $email = I('post.email');
    ...
    public function edit_mobile(){
        $mobile = isset($_POST['mobile']) ? in($_POST['mobile']) : null;
        $user_id = isset($_POST['uid']) ? intval($_POST['uid']) : $this->user_id;
        if($mobile == null){
            echo json_encode(array('status'=>false, 'content'=>'手机号码为空')); exit();
        }
        if (is_mobile($mobile)) { //  手机号码格式正确
            $sql = "SELECT * from ".$this->model->pre."users where user_name='".$mobile."' or mobile_phone='".$mobile."' and user_id <> ".$user_id;
            $res = $this->model->query($sql);
            if(empty($result)){
                $sql = 'update ' . $this->model->pre . "users  set `mobile_phone` = '".$mobile."' where user_id=".$user_id;
                $result = $this->model->query($sql);
                if($result){
                    echo json_encode(array('status'=>true, 'content'=>'手机号码修改成功', 'mobile'=>$mobile)); exit();
                }else{
                    echo json_encode(array('status'=>false, 'content'=>'手机号码修改失败')); exit();
                }

            }else{
                echo json_encode(array('status'=>false, 'content'=>'手机号码已存在')); exit();
            }
            
        }else{
            echo json_encode(array('status'=>false, 'content'=>'手机号码格式错误')); exit();
        }
    ...
    public function edit_password(){
        $old_password = isset($_POST['oldpwd']) ? in($_POST['oldpwd']) : null;
        $new_password = isset($_POST['newpwd']) ? in($_POST['newpwd']) : null;
        $rpt_password = isset($_POST['rptpwd']) ? in($_POST['rptpwd']) : null;
        
        $user_id = isset($_POST['uid']) ? intval($_POST['uid']) : $this->user_id;
        if($old_password == null){
            echo json_encode(array('status'=>false, 'content'=>'旧密码错误')); exit();
        }

        if($new_password == null){
            echo json_encode(array('status'=>false, 'content'=>'新密码错误')); exit();
        }

        if($new_password !== $rpt_password){
            echo json_encode(array('status'=>false, 'content'=>'新密码不一致')); exit();
        }

        // by fjw :这个self::$user->get_profile_by_id 用的是  public_html/plugins/integrates里的
        $user_info = self::$user->get_profile_by_id($user_id); // 用户信息
        $encry_pwd = md5($old_password);
        if(!empty($user_info['ec_salt'])){ // 没有盐
            $encry_pwd = md5($encry_pwd.$user_info['ec_salt']);
        }
        
        if($encry_pwd === $user_info['password']){ // 旧密码验证通过
            
            $data['ec_salt'] = rand(1, 9999);
            $data['password'] = md5(md5($new_password).$data['ec_salt']);
            
            $sql = 'update ' . $this->model->pre . "users  set `password` = '" . $data['password'] . "' , `ec_salt` = ".$data['ec_salt']." WHERE `user_id`=" .$user_id;

            $result = $this->model->query($sql);
            if($result){
                echo json_encode(array('status'=>true, 'content'=>'密码更新成功')); exit();
            }else{
                echo json_encode(array('status'=>false, 'content'=>'更新密码失败')); exit();
            }
...
>>>>>>> 81068c63c712f63d925e3854a80b9bf5f829b078
