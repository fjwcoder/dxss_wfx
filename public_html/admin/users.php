...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name,sex,birthday,mobile_phone,email, user_rank,is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
    ...
    for ($i=0; $i<$count; $i++)
    {
        $user_list[$i]['reg_time'] = local_date($GLOBALS['_CFG']['date_format'], $user_list[$i]['reg_time']);
		 ...
		 $sql = "SELECT nickname from".$GLOBALS['ecs']->table('wechat_user')." WHERE ect_uid  = $id";
         $jian_row = $GLOBALS['db']->getRow($sql);
		 $user_list[$i]['nickname']=$jian_row['nickname'];
		 $id=$user_list[$i]['user_rank'];
		  $sql = "SELECT rank_name from".$GLOBALS['ecs']->table('user_rank')." WHERE rank_id  = $id";
         $jian_row = $GLOBALS['db']->getRow($sql);
		  $user_list[$i]['rank_name']=$jian_row['rank_name'];
		  if (empty($user_list[$i]['rank_name']))
		      $user_list[$i]['rank_name']="普通会员";
    }
